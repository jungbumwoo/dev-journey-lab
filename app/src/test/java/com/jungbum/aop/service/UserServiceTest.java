...
import static com.jungbum.service.UserServiceImpl.MIN_LOGCOUNT_FOR_SILVER;
import static com.jungbum.service.UserServiceImpl.MIN_RECCOMEND_FOR_GOLD;
...
public class UserServiceTest {
    ...
    static class TestConfig {
        ...
        // 1. Advisor interface를 구현한 것을 찾음.
        // 2. 생성되는 모든 빈에 대해 포인트컷을 적용하면서 적용 대상을 선정
        // 3. 프록시 선정 대상이면 원래 빈 object를 교체함
        // upgradeAllOrNothing Test에서 트랜잭션 적용 여부를 검증함
        @Bean
        ...
        public UserService testUserService(UserDao userDao, MailSender mailSender) {
            UserService testServiceImpl = new TestUserServiceImpl(userDao, mailSender);
            return testServiceImpl;
        }

        @Bean
        public UserService userService(UserDao userDao, MailSender mailSender) {
            UserService userServiceImpl = new UserServiceImpl(userDao, mailSender);
    ...
    public void setUp() {
        users = Arrays.asList(
                new User("bumjin", "abcd", "p1", "user1@ksug.org", Level.BASIC, MIN_LOGCOUNT_FOR_SILVER-1, 0),
                new User("joytouch", "efg", "p2", "user2@ksug.org", Level.BASIC, MIN_LOGCOUNT_FOR_SILVER, 0),
                new User("erwins", "hijk", "p3", "user3@ksug.org", Level.SILVER, 60, MIN_RECCOMEND_FOR_GOLD-1),
                new User("madnite1", "lmnop", "p4", "user4@ksug.org", Level.SILVER, 60, MIN_RECCOMEND_FOR_GOLD),
                new User("green", "qrst", "p5", "user5@ksug.org", Level.GOLD, 100, Integer.MAX_VALUE)
        );
    }
    ...
    public void upgradeAllOrNothing() {
        ...
        System.out.printf("users.get(1): %s%n", users.get(1));
        ...
        System.out.printf("after users.get(1): %s%n", users.get(1));
